...
from app.utils.csv_loader import df_to_log_entries, load_log_batch
...
def _progress_event(node_name: str, delta: dict[str, Any]) -> dict[str, Any]:
...
async def _run_investigation_task(run_id: str, df: pd.DataFrame) -> None:
    """Background task: stream the investigation graph, recording progress + the
    final InvestigationState into the RunStore as the run proceeds."""
    ...
    config = {"configurable": {"pack": GOVERNANCE_PACK, "df": df, "metrics": metrics}}

    try:
        async for chunk in graph.astream(state, config=config, stream_mode="updates"):
            for node_name, delta in chunk.items():
                state.update(delta)
                run_store.append_progress(run_id, _progress_event(node_name, delta))
...
def _get_run_or_404(run_id: str) -> InvestigationRun:
...
async def investigate(session_id: str) -> InvestigateResponse:
    ...
    asyncio.create_task(_run_investigation_task(run_id, session.df))
